...
### GUI Completely Broken - Clicks Do Nothing

**Symptom:** Clicking ANY menu item does nothing. Metrics show flat lines. Page appears loaded but non-functional.

**Diagnosis:**
1. Check debug logs: `GET /api/ai-debug/exec` with `tail -50 .../Shellder/logs/debuglog.txt`
2. Look for: `⚠️ Expected endpoints not called: /api/status, /api/services...`
3. This means JavaScript loaded but initialization failed

**Common Causes:**
1. **Function wrapper reassigning global function**
   ```javascript
   // THIS BREAKS ONCLICK HANDLERS:
   const original = typeof navigateTo === 'function' ? navigateTo : null;
   window.navigateTo = function(page) { original(page); };
   ```
2. **Syntax error in script.js** preventing full load
3. **Missing page in titles object** (won't break clicks, but shows wrong title)

**Fix Pattern:**
- Don't wrap functions - modify the original directly
- Add new page handling inside existing `navigateTo()` function
- Use defensive checks: `if (typeof loadFunc === 'function') loadFunc();`
